...
uses
  ...
  IBQuery, StdCtrls, CheckLst, Menus, ImgList, ShellAPI, WordXP, DateUtils,Math;
...
var
  ...
  putchshab, putchrezult, Datename,putchrezultcab,datemodifitext,EmailAdress: string;
  i, c, j: integer;
...
if (Form1.CheckDeckList.Checked[2]=true) then begin
if str<>'' then
  str:=str+', ';
 str:=str+'третю';
end;
...
begin
n:=0;
  if (Form1.CheckDeckList.Checked[0]=true)
  then
  n:=n+1;
   if (Form1.CheckDeckList.Checked[1]=true)
  then
  n:=n+1;
   if (Form1.CheckDeckList.Checked[2]=true)
  then
  n:=n+1;
...
begin
  ...
  if (dwRet <> SUCCESS_SUCCESS) then begin
    MessageBox(Handle,PChar('Помилка: Outlook Express.'+#13#10+'Неможливо відкрити MAPI сесію.'+#13#10+'Можливо mapi.dll була пошкоджена.'+#13#10+'Зверніться к системному адміністратору.'),PChar('Error'),MB_ICONERROR or MB_OK);
  end else begin
...
begin
  ...
  begin
   ...
   if (CheckDeckList.Checked[0] = false) and (CheckDeckList.Checked[1] = false) and (CheckDeckList.Checked[2] = false) then
      begin
...
{$ENDIF}
  ...
  tmp2 := wdFindContinue;
    if ((CBMonList.Checked) and (ComboMonList.ItemIndex >= 0)) then
      tmp3 := Mon_ukr[ComboMonList.ItemIndex] + ' '+FormatDateTime('yyyy', DTProgn.Date)
 else
  tmp3 := '';
  tmp4 := wdReplaceAll;
  ...
  begin
    ...
    // ===========================================================
        ...
        IBQZABEZP.SQL.Clear;
        IBQZABEZP.SQL.Add
          ('select Q' + IntToStr(ComboMonList.ItemIndex + 1)+'  from q_zabezp q where q.post_index="' + ind_st +
            '" and q.proc="ser"');
        IBQZABEZP.Open;
            if not IBQZABEZP.FieldByName('Q' + IntToStr(ComboMonList.ItemIndex + 1)).IsNull then
              Cell(curCol, 3).Range.Text := FormatPripliv
                (IBQZABEZP.FieldByName('Q' + IntToStr(ComboMonList.ItemIndex + 1)).AsFloat);
          //-----------------------------------------------------------
        IBQZABEZP.Close;
        IBQZABEZP.SQL.Clear;
        IBQZABEZP.SQL.Add
        ...
        IBQZABEZP.Open;
          ...
          //-----------------------------------------------------------
              ...
              IBQPripliv.SQL.Add
                ('select AVG(qq_middprev) as QQ from GYDRO_4_5' +
...
//  Ïîäïèñü
 ...
 begin     //1
  if (ComboBox2.Text=ComboBox2.Items[0]) then  path_signature:=putchshab+'\'+ 'Boss1.bmp'
  else
    ...
    //äîáàâëÿåì ôàéë ñ map
        wdUnit := true;
        with(WordApp.Selection.InlineShapes
          .AddPicture(path_signature, EmptyParam, wdUnit, EmptyParam))do
        begin
    ...
    WordApp.Selection.Find.ExecuteOld(wdUnit, EmptyParam, tmp1, EmptyParam,
      ...
      if copy(parForm, 1, 1) = '1' then
        ...
        begin
          ...
          begin
          if Form1.CheckDeckList.Checked[0]=true then  begin
            Cell(curCol, 1).Range.Text := Mon_ukr[ComboDecMonList.ItemIndex]
              + ', ' + Dec_ukr[0];
               curCol := curCol + 1;
          end;
               if Form1.CheckDeckList.Checked[1]=true then  begin
            Cell(curCol, 1).Range.Text := Mon_ukr[ComboDecMonList.ItemIndex]
              + ', ' + Dec_ukr[1];
                curCol := curCol + 1;
               end;
...
on Exception do begin ShowMessage('Помилка відкриття поштової програми.'); exit; end;
end;
mail.Free;
mail_1.Free;
end;
...
begin
CopyFile(PChar(putchrezult + '\' + m_ReportFileName),PChar(putchrezultcab + '\' + m_ReportFileName),false);
if FileExists(putchrezultcab + '\' + m_ReportFileName) then ShowMessage('Файл відправлено у кабінет !');
end else ShowMessage('Файл для відправлення в кабінет відсутній, сформуйте його!');
end;
...
begin
  ...
  putchrezult := IniGetStringValue(iniPath, 'Options', 'result' + two_parram,
    default);
  putchrezultcab := IniGetStringValue(iniPath, 'Options', 'cabinet' + two_parram,
    default);
  //-------------
...
begin
  ...
  begin
    ...
    begin
      ...
      NFile.Caption :=
        'Назва файлу результату:         ' + m_ReportFileName;
      NameDirect.Caption := 'Назва дерикторії результату:    ' + putchrezult;
      NameDirectCab.Caption:='Назва дерикторії для кабінету:    '+putchrezultcab;
      //---------------------
      if FileExists(putchrezult+'\'+m_ReportFileName) then
      begin
      GetFileAttributesEx(PChar(putchrezult+'\'+m_ReportFileName), GetFileExInfoStandard, @fi);//Заносимо назву файла у змінну @fi
FileTimeToLocalFileTime(fi.ftLastWriteTime,FileLocalModify);
FileTimeToSystemTime(FileLocalModify,FileSystemModify);
datemodifitext:=FormatDateTime('dd.mm.yyyy р. hh:mm:ss годин', SystemTimeToDateTime(FileSystemModify));
  DeteModifi.Caption:='Дата модифікації:    '+datemodifitext;
      end else    DeteModifi.Caption:='Дата модифікації:';
  // -----------------------------
  EmailAdress:=IBViewForm.FieldByName('EMAIL').AsString;
  NameEmailadress.Caption:='Адреси електронної пошти:         '+EmailAdress;
    end;
    IBViewForm.Free;
  end;
end;
