...
begin
 ...
 procedure TForm1.footer();
 var
  namePres, path_signature:string;
 begin
     if (copy(Nach1, Pos('#', Nach1) + 1, (Length(Nach1))) = ComboBox2.Text) then
    namePres := copy(Nach1, 1, Pos('#', Nach1) - 1)
  else
    namePres := copy(Nach2, 1, Pos('#', Nach2) - 1);
  wdUnit := '#President';
  tmp1 := true;
  tmp2 := wdFindContinue;
  tmp3 := namePres;
...
begin
  ...
  if (parForm = 'bmp') and (CBMonList.Visible) and (CBMonList.Checked) then
      begin
        if (ComboMonList.ItemIndex = -1) then
        begin
          ShowMessage('Не вибрано період прогнозу');
  ...
  if parForm = 'dmp' then
    ...
    Progn_bok_mon
  else if parForm = 'le1' then
  led
    else if parForm = 'le2' then
   led
     else if parForm = 'le3' then
     led
             else if parForm = 'le4' then
     led
...
function Tform1.deleteTable(tableIndex,  upDeleteCount,dwnDeleteCount:integer; tableHead:string ):boolean ;
var
  Extend:OLEVariant;
begin
