...
{$ENDIF}
  ...
  if ((CBMonList.Checked) and (ComboMonList.ItemIndex >= 0)) then
    tmp3 := Mon_ukr[ComboMonList.ItemIndex] + ' ' + FormatDateTime
      ('yyyy', DTProgn.Date)
  else
    tmp3 := '';
  ...
  begin
    ...
    begin
      ...
      IBQZABEZP.Close;
      IBQZABEZP.SQL.Clear;
      IBQZABEZP.SQL.Add('select avg(q) as q from (select Q3 as q from  q_zabezp q where q.post_index=81921'+
            ' and q.proc="ser"'+
            ' union'+
            ' select Q4 as q from  q_zabezp q where q.post_index=81921'+
            ' and q.proc="ser") as q');
      IBQZABEZP.Open;
      if not IBQZABEZP.FieldByName('Q')
        .IsNull then
        Cell(curCol, 3).Range.Text := FormatPripliv
          (IBQZABEZP.FieldByName('Q')
            .AsFloat);
      // -----------------------------------------------------------
      IBQZABEZP.Close;
      IBQZABEZP.SQL.Clear;
      IBQZABEZP.SQL.Add('select min(q) as q from (select Q3 as q from  q_zabezp q where q.post_index=81921'+
            ' and q.proc="qmin"'+
            ' union'+
            ' select Q4 as q from  q_zabezp q where q.post_index=81921'+
      ...
      IBQZABEZP.Close;
      IBQZABEZP.SQL.Clear;
      IBQZABEZP.SQL.Add('select max(q) as q from (select Q3 as q from  q_zabezp q where q.post_index=81921'+
            ' and q.proc="qmax"'+
            ' union'+
            ' select Q4 as q from  q_zabezp q where q.post_index=81921'+
      ...
      IBQZABEZP.Open;
      if (not IBQZABEZP.FieldByName('Q3')
        .IsNull) and (not IBQZABEZP.FieldByName('Q4')
        .IsNull) then
        avg:= IBQZABEZP.FieldByName('Q3')
            .AsInteger+ IBQZABEZP.FieldByName('Q4')
            .AsInteger;
