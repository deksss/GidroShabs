...
begin
  ...
  begin
    Zakladka := '';
    val := '';
    Zakladka := Group_Zakladka_Query.FieldByName('type_').AsString + 'ъ' +
      Zakladka_Elementi_Query.FieldByName('VALUE_').AsString + 'ъ' + IntToStr
      (Prognozyear.ItemIndex);
    val := Zakladka_Elementi_Query.FieldByName('VALUE_').AsString;
    if (val = 'yy') or (val = 'yyyy') then
      Zakladka := Zakladka + 'ъ' + IntToStr(Prognozyear.ItemIndex);
    if Yazik.Text = 'Російська' then
      Zakladka := Zakladka + 'ъ' + IntToStr(Prognozyear.ItemIndex);
    if Yazik.Text = 'Українська' then
      Zakladka := Zakladka + 'ъukrъ';
    if Yazik.Text = 'Російська' then
      Zakladka := Zakladka + 'ъrusъ';
  ...
  begin
    ...
    Zakladka := Group_Zakladka_Query.FieldByName('type_')
      .AsString + 'ъ' + PrognozQuery.FieldByName('VALUE_').AsString;
    ...
    begin
      if (CheckBox1.Checked) then
        Zakladka := Zakladka + 'ъ1ъ' + DecListQuery.FieldByName('VALUE_')
          .AsString
      else
        Zakladka := Zakladka + 'ъ0';
    end;
    if (val1 = 'monper') then
      Zakladka := Zakladka + 'ъ' + IntToStr(monbeg.ItemIndex) + 'ъ' + IntToStr
        (monend.ItemIndex);
    if Yazik.Text = 'Українська' then
      Zakladka := Zakladka + 'ъukrъ';
    if Yazik.Text = 'Російська' then
  ...
  begin
    ...
    Zakladka := Group_Zakladka_Query.FieldByName('type_').AsString + 'ъ' +
      ...
      .AsString + 'ъ' + VodocheQuery.FieldByName('POST_INDEX')
      .AsString + 'ъ' + PrognozQuery.FieldByName('VALUE_').AsString;
    val := PrognozQuery.FieldByName('TYPE_').AsString;
    val1 := PrognozQuery.FieldByName('VALUE_').AsString;
    if ((val = 'wmon') and (val1 <> 'monper')) then
...
begin
  ...
  begin
    ...
    // поиск на какой закладке находится курсор начало
    ...
    // поиск на какой закладке находится курсор конец
  end
  else
  begin
    MessageDlg('Увага:' + #13#10 + 'Активних документів незнайдено', mtWarning,
      [mbOK], 0);
  end;
...
begin
  if MsWord.Documents.Count > 0 then
  begin
    SendMessageUser('Зачекайте', Form2, true);
    ...
    // ---------------------------------------------
    if pos('wDate', copy(BookmarksName, 1, pos('ъ', BookmarksName) - 1))
      > 0 then
    begin // Закладка Даты начало
      Locate(Group_Zakladka_Query, 'type_', 'wDate', [loPrimary]);
      // ищем нужную запись
      Group_ZakladkaClick(Sender);
      Group_Zakladka.KeyValue := Group_Zakladka_Query.FieldByName('name_uk')
        .AsString;
      BookmarksName := copy(BookmarksName, pos('ъ', BookmarksName) + 1, length
          (BookmarksName));
      format := copy(BookmarksName, 1, pos('ъ', BookmarksName) - 1);
      ...
      begin
        progy := '';
        progy := copy(BookmarksName, 1, pos('ъ', BookmarksName) - 1);
        if progy <> '' then
          Prognozyear.ItemIndex := StrToInt(progy);
        BookmarksName := copy(BookmarksName, pos('ъ', BookmarksName) + 1, length
            (BookmarksName));
      end;
      language := copy(BookmarksName, 1, pos('ъ', BookmarksName) - 1);
      if language = 'ukr' then
        Yazik.Text := 'Українська';
      if language = 'rus' then
        Yazik.Text := 'Російська';
      // YazikChange(Sender);
    end // Закладка Даты конец
    else // --------------------------------------
      if pos('wNPer', copy(BookmarksName, 1, pos('ъ', BookmarksName) - 1))
      > 0 then
    begin // Закладка Бюлетень начало
      ...
      BookmarksName := copy(BookmarksName, pos('ъ', BookmarksName) + 1, length
          (BookmarksName));
      numper := copy(BookmarksName, 1, pos('ъ', BookmarksName) - 1);
      ...
      if (PrognozQuery.FieldByName('value_').AsString = 'monper') then
        ...
        begin
          mbeg := '';
          mbeg := copy(BookmarksName, 1, pos('ъ', BookmarksName) - 1);
          if mbeg <> '' then
            monbeg.ItemIndex := StrToInt(mbeg);
          BookmarksName := copy(BookmarksName, pos('ъ', BookmarksName) + 1,
            length(BookmarksName));
          mend := '';
          mend := copy(BookmarksName, 1, pos('ъ', BookmarksName) - 1);
          if mend <> '' then
      ...
      begin
        decm := '';
        decm := copy(BookmarksName, 1, pos('ъ', BookmarksName) - 1);
        if decm = '1' then
        begin
...
//          PrognozzaClick(Sender);
      ...
      if language = 'rus' then
        Yazik.Text := 'Російська';
      // YazikChange(Sender);
    end // Закладка Метео информация конец
    else // --------------------------------------
    if pos('Gyd', copy(BookmarksName, 1, pos('ъ', BookmarksName) - 1))> 0 then
    begin // Закладка Бюлетень начало
      ...
      BookmarksName := copy(BookmarksName, pos('ъ', BookmarksName) + 1, length
          (BookmarksName));
      // -------------------------------------
      vodche := copy(BookmarksName, 1, pos('ъ', BookmarksName) - 1); ;
...
//          PrognozzaClick(Sender);
    ...
    SendMessageUser('Зачекайте', Form2, false);
  end;
...
begin
  if MessageDlg('Видалити закладку?', mtWarning, [mbYes, mbNo], 0) = mrYes then
  begin
    SendMessageUser('Зачекайте', Form2, true);
...
begin
  ...
  try
    begin
      ...
      begin // 1 begin
        if MessageDlg
          ('Увага : така закладка вже існує.' + #13#10 + 'Дублювати?',
          mtWarning, [mbYes, mbCancel], 0) = mrYes then
        begin // 2 begin
          for i := 1 to 10 do
            if Doc.Bookmarks.Exists(Zakladka) then
              Insert('1', Zakladka, pos('ъ', Zakladka));
...
begin
  ...
  begin
    ...
    MessageDlg('Увага:' + #13#10 + 'Невідомий тип інформації', mtWarning,
      [mbOK], 0);
end;
...
begin
  ...
  begin
    ...
    VodocheQuery.SQL.Text :=
      'select *  from CAT_POSTM where  TYPEPOST="БФ" order by CPM_NAME';
    VodocheQuery.Open;
    VodocheQuery.Last;
    VodocheQuery.First;
    Vodoche.KeyValue := VodocheQuery.FieldByName('CPM_NAME').AsString;
    Label4.Caption := 'Водосховища:';
    Label4.Visible := true;
    Vodoche.Visible := true;
    // ----------------
    PrognozQuery.Close;
